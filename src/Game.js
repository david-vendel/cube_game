...
class Game extends Component {
    ...
    updateGraph() {
        ...
        for (var r = 0; r < this.state.sizey; r++) {
            for (var s = 0; s < this.state.sizex; s++) {
                if (this.state.c[r][s] == '#111') {
                    black += 1;
                } else if (this.state.c[r][s] == 'limegreen') {
                    green += 1;
                } else if (this.state.c[r][s] == 'red') {
                    red += 1;
                } else if (this.state.c[r][s] == 'royalblue') {
                    royalblue += 1;
                } else if (this.state.c[r][s] == 'black') {
                    black += 1;
                } else if (this.state.c[r][s] == 'violet') {
                    violet += 1;
                } else if (this.state.c[r][s] == '#eee') {
                    silver += 1;
                } else if (this.state.c[r][s] == 'orchid') {
                    orchid += 1;
                }
            }
        }
            ...
            setTimeout(() => {
                ...
                for (let y9 = 0; y9 < this.state.sizey; y9++) {
                    for (let yy9 = 0; yy9 < this.state.sizex; yy9++) {
                        if (
                            this.state.p[y9][yy9] === 6 &&
                            this.state.c[y9][yy9] === 'royalblue'
                        ) {
                            var qq = this.state.queue;
            ...
            setTimeout(() => {
                ...
                for (let y9 = 0; y9 < this.state.sizey; y9++) {
                    for (let yy9 = 0; yy9 < this.state.sizex; yy9++) {
                        if (
                            this.state.p[y9][yy9] === 6 &&
                            (this.state.c[y9][yy9] === 'orchid' ||
                                this.state.c[y9][yy9] === 'violet')
                        ) {
                            var qq = this.state.queue;
                ...
                while (omc > 0) {
                    var a2 = Math.floor(
                        this.state.sizey * Math.pow(Math.random(), 2)
                    );
                    var b2 = Math.floor(
                        this.state.sizex * Math.pow(Math.random(), 2)
                    );
                    if (
                        (Math.random() > 0.8 || this.state.p[a2][b2] > 4) &&
                        ((this.state.c[a2][b2] == 'orchid' &&
                            ((a2 + 1 < this.state.sizey &&
                                this.state.c[a2 + 1][b2] != 'orchid') ||
                                (b2 + 1 < this.state.sizex &&
                                    this.state.c[a2][b2 + 1] != 'orchid') ||
                                (a2 > 0 &&
                                    this.state.c[a2 - 1][b2] != 'orchid') ||
                                (b2 > 0 &&
                                    this.state.c[a2][b2 - 1] != 'orchid'))) ||
                            (this.state.c[a2][b2] == 'violet' &&
                                ((a2 + 1 < this.state.sizey &&
                                    this.state.c[a2 + 1][b2] != 'violet') ||
                                    (b2 + 1 < this.state.sizex &&
                                        this.state.c[a2][b2 + 1] != 'violet') ||
                                    (a2 > 0 &&
                                        this.state.c[a2 - 1][b2] != 'violet') ||
                                    (b2 > 0 &&
                                        this.state.c[a2][b2 - 1] != 'violet'))))
        ...
        if (!this.state.autoplay) {
            ...
            if (this.freeze == 1) {
                ...
                //bigturn is used to allow user click even before computer moves are finished. his move is recorded to queue and used at the end, but only once. bigturn prevents using more times.
                    ...
                    this.state.bigturn == true &&
                    this.state.queue.length < 40
                ) {
                    var qq = this.state.queue;
    ...
    explode(a, b, ccc) {
        ...
        if (a > 0) {
            qq.unshift([a - 1, b, ccc, 'explode']);
        }
        if (b > 0) {
            qq.unshift([a, b - 1, ccc, 'explode']);
        }
        if (a < this.state.sizey - 1) {
            qq.unshift([a + 1, b, ccc, 'explode']);
        }
        if (b < this.state.sizex - 1) {
            qq.unshift([a, b + 1, ccc, 'explode']);
        }
            ...
            var buttonOne = (
                <div>
                    <button
                        ...
                        style={{
                            backgroundColor: this.state.c[a][b],
                            color: this.state.d[a][b],
                            fontSize: this.state.diceGrid[a][b],
                        }}
                ...
                <div>
                    <button
                        ...
                        style={{
                            backgroundColor: this.state.c[a][b],
                            color: this.state.d[a][b],
                            fontSize: this.state.buttonGrid[a][b],
                        }}
                        onClick={(e) => this.boxClicked(e, a, b)}
                    >
                        {this.state.p[a][b]}
    ...
    render() {
        ...
        var versionSpan = (
            <span className="version" onClick={this.versionChange}>
                {this.state.version}
            </span>
        );
        ...
        if (this.state.green == this.state.sizex * this.state.sizey) {
            intro = (
                <div className="riadok intro">
                    <span style={{ fontSize: 45, color: 'green' }}>
                        YOU WON!
                    </span>
                    {versionSpan}
                </div>
            ...
            if (this.state.green == 0) {
                ...
                if (this.state.turn) {
                    ...
                    intro = (
                        <div className="riadok intro">
                            You are{' '}
                            <span style={{ color: 'darkred' }}>green</span>.
                            Conquer <br />
                            the whole area to win.{versionSpan}
                        </div>
                    );
        ...
        var counter = (
            <div>
                <a href="http://www.simple-counter.com/" target="_blank">
                    <img
                        src="http://www.simple-counter.com/hit.php?id=zveaxcq&nd=8&nc=11&bc=4"
                        border="0"
                        alt="Hit Counter"
                    />
                </a>
        ...
        var graph1 = (
            <div
                style={{
                    height: 18,
                    width: (70 * this.state.green) / this.state.sizey,
                }}
                className="stlpec green"
            ></div>
        );
        var graph2 = (
            <div
                style={{
                    visibility: this.state.visible[1],
                    height: 18,
                    width: (70 / this.state.sizey) * this.state.red,
                }}
                className="stlpec red"
            ></div>
        );
        var graph3 = (
            <div
                style={{
                    visibility: this.state.visible[2],
                    height: 18,
                    width: (70 / this.state.sizey) * this.state.royalblue,
                }}
                className="stlpec royalblue"
            ></div>
        );
        var graph4 = (
            <div
                style={{
                    visibility: this.state.visible[3],
                    height: 18,
                    width: (70 / this.state.sizey) * this.state.violet,
                }}
                className="stlpec violet"
            ></div>
        );
        var graph5 = (
            <div
                style={{
                    visibility: this.state.visible[4],
                    height: 18,
                    width: (70 / this.state.sizey) * this.state.orchid,
                }}
                className="stlpec orchid"
            ></div>
        );

        var g = [];

        if (this.state.visible[0] == 'visible') {
            g.push(graph1);
        }
        if (this.state.visible[1] == 'visible') {
            g.push(graph2);
        }
        if (this.state.visible[2] == 'visible') {
            g.push(graph3);
        }
        if (this.state.visible[3] == 'visible') {
            g.push(graph4);
        }
        if (this.state.visible[4] == 'visible') {
            g.push(graph5);
        }

        var graph = <div className="graph">{g}</div>;
        ...
        return (
            <div className="App" style={{ width: 70 * this.state.sizex }}>
                ...
                <div className="">
                    ...
                    <button
                        title="1 opponent who has 2 moves. No intelligence."
                        className="difficultyButton"
                        style={{ backgroundColor: this.state.easy }}
                        onClick={this.easy}
                    >
                        {' '}
                        EASY{' '}
                    ...
                    <button
                        title="2 opponents having 2 moves each. Blue will go after 6s first."
                        className="difficultyButton"
                        style={{ backgroundColor: this.state.medium }}
                        onClick={this.medium}
                    >
                        {' '}
                        MEDIUM{' '}
                    ...
                    <button
                        title="Red has 2 moves, blue and violet 1.5 moves each. Violet will only play at the borders, prefer to go after 5s and 6s and advance from left top to right bottom."
                        className="difficultyButton"
                        style={{ backgroundColor: this.state.hard }}
                        onClick={this.hard}
                    >
                        {' '}
                        HARD{' '}
                    ...
                    <button
                        title="All 3 opponents have 2 moves each."
                        className="difficultyButton imba"
                        style={{
                            backgroundColor: this.state.imba,
                            color: this.state.imbaColor,
                        }}
                        onClick={this.imba}
                    >
                        {' '}
                ...
                <div className="resetContainer">
                    ...
                    <div
                        className={this.state.diceClass}
                ...
                <div className="signature">
                    ...
                    <div>
                        <a href="https://github.com/justdvl" target="blank">
                            © 2020 David Vendel
                        </a>
                    </div>
                </div>

                <div className="info">Firing {this.state.info} at once</div>
            </div>
        );
    }
}
