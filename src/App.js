...
class App extends Component {
  ...
  componentDidMount() {
    this.interval = setInterval(this.changeActiveMedia.bind(this), this.state
      .render);
    this.updateGraph()
  }
  ...
  makeButtonLine(a) {
    ...
    var x = < div className = "riadok"
    style = {
      {
        width: 70 * this.state.sizex
      }
    } > {
      arr
    } < /div>
  ...
  render() {
    ...
    var versionSpan = < span className = "version"
    onClick = {
      this.versionChange
    } > {
      this.state.version
    ...
    var rangeSpan = < span className = "rangespan" > < input type = "range"
    step = "20"
    defaultValue = "60"
    style = {
      {
    ...
    if (this.state.green == this.state.sizex * this.state.sizey) {
      intro = < div className = "riadok intro" >
        < span style = {
          {
            fontSize: 45,
            color: "green"
          }
        } > YOU WON! < /span>{versionSpan} < /div >
    }
    else {
      if (this.state.green == 0) {
        ...
        if (this.state.turn) {
          intro = < div className = "riadok intro" >
            ...
            } > green < /span>. Conquer <br/ > the whole area to win. {
              versionSpan
            } < /div>
        }
        else {
    ...
    var counter = < div >
      < a href = "http://www.simple-counter.com/"
    target = "_blank" > < img src =
      "http://www.simple-counter.com/hit.php?id=zveaxcq&nd=8&nc=11&bc=4"
    border = "0"
    alt = "Hit Counter" / > < /a> < /div >
      ...
      return ( < div className = "App"
        ...
        < label > Autoplay < input name = "autoplay"
    type = "checkbox"
    checked = {
      this.state.autoplay
    }
    ...
    < /div> < button title = "1 opponent who has 2 moves. No intelligence."
    className = "difficultyButton"
    style = {
      {
        backgroundColor: this.state.easy
      }
    }
    onClick = {
        this.easy
      } > EASY <
      /button> < button title = "2 opponents having 2 moves each. Blue will go after 6s first."
    className = "difficultyButton"
    style = {
      {
        backgroundColor: this.state.medium
      }
    }
    onClick = {
        this.medium
      } > MEDIUM <
      /button> < button title = "Red has 2 moves, blue and violet 1.5 moves each. Violet will only play at the borders, prefer to go after 5s and 6s and advance from left top to right bottom."
    className = "difficultyButton"
    style = {
      {
        backgroundColor: this.state.hard
      }
    }
    onClick = {
        this.hard
      } > HARD <
      /button> < button title = "All 3 opponents have 2 moves each."
    ...
    className = "stlpec green" > < /div> < div style = { {
    height: 18,
      width: (70 / this.state.sizey) * this.state.red
  }
}
className = "stlpec red" > < /div> < div style = { {
height: 18,
  width: (70 / this.state.sizey) * this.state.royalblue
}
}
className = "stlpec royalblue" > < /div>
...
< div > < a href = "https://github.com/justdvl"
target = "blank" > ©2018 David Vendel < /a></div >
  < /div>
